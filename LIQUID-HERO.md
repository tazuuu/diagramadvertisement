...
| `assets/hero-liquid.png` | The orange/teal frame (`download (1).png`) |
...
<div class="hero-liquid" data-liquid-hover
     data-src="assets/hero-liquid.png"
...
`assets/hero-liquid.png` is **1898×839**, cropped from the original 2282×1011
supplied file. That file had bars baked into it on all four sides — 84px top and
bottom, 190px left and right — so `object-fit: cover` was filling the hero with
a framed picture rather than with the photograph, and the hero never reached the
viewport edges. The bars are gone; the image is now full-bleed.

If you replace this asset, check it for the same problem before dropping it in,
and keep the `width`/`height` attributes on the `<img>` in sync with the real
pixel dimensions (they were previously declared as 2000×855 for a 2282×1011
file, which misstated the intrinsic aspect ratio).

## Known trade-off

The shot is very wide (2.26:1). On a narrow portrait phone, `cover` crops it to
the horizontal centre, which is the darker side of this particular photograph —
so on mobile it reads as atmospheric texture rather than as a recognisable
frame. Two ways to fix that if you want the subject visible on phones: supply a
taller crop of the same shot and swap it in under a media query, or bias the
sampling toward the bright side of the frame on narrow viewports.
